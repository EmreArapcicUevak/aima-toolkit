...
def test_CSP_constraints():
  ...
  assert equal_A_B_constraint in test_csp.constraints["A"]
  assert equal_A_B_constraint in test_csp.constraints["B"]
  assert equal_A_B_constraint not in test_csp.constraints["C"]

  # test bad assignment (out of domain)
  with pytest.raises(AssertionError):
    invalid_assignment = { "A": 1, "B": 4, "C": 7}
    test_csp.is_consistent("A", invalid_assignment)

  # test good assignment (inconsistent assignment)
  inconsistent_assignment = { "A": 1, "B": 2, "C": 4}
  inconsistent_assignment2 = { "A": 2, "B": 3, "C": 4}
  assert test_csp.is_consistent("A", inconsistent_assignment) == False
  assert test_csp.is_consistent("A", inconsistent_assignment2) == False

  # test good assignment (consistent assignment)
  consistent_assignment = { "A": 2, "B": 2, "C": 4}
  consistent_assignment2 = { "A": 2, "B": 2, "C": 5}
  assert test_csp.is_consistent("A", consistent_assignment) == True
  assert test_csp.is_consistent("A", consistent_assignment2) == True
